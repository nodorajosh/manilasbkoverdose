...
import DiscountUsageChart from "@/components/admin/discount-usage-chart";
...
export default function DiscountsAdmin() {
    ...
    }>({ open: false, action: null, discount: null });

    const toast = useToast();
    const utils = trpc.useUtils();

    const listQuery = trpc.admin.discounts.list.useInfiniteQuery(
    ...
    const invalidate = () => {
        void utils.admin.discounts.list.invalidate();
        void utils.admin.discounts.usageStats.invalidate();
    };

    const onCreated = (d?: Discount) => {
        toast.push({ title: "Created", message: d?.code ?? "New discount", level: "success" });
        setShowCreateModal(false);
        invalidate();
    };

    const onSaved = (d?: Discount) => {
        ...
        try {
            if (action === "archive") {
                const res = await fetch("/api/admin/discounts", {
                    ...
                    body: JSON.stringify({ discountId: discount._id, active: false }),
                });
                ...
                const res = await fetch("/api/admin/discounts", {
                    method: "DELETE",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ discountId: discount._id, hard: true }),
                });
    ...
    const restore = async (d: Discount) => {
        try {
            const res = await fetch("/api/admin/discounts", {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ discountId: d._id, active: true }),
            });
        ...
        <div className="space-y-6">
            ...
            <div>
                {listQuery.isLoading ? (
                    ...
                    <div className="text-sm text-red-400">Failed to load discounts.</div>
                ) : discounts.length === 0 ? (
                            ...
                            <div key={d._id} className="p-3 border rounded flex items-center justify-between">
                                <div>
                                    <div className="font-semibold">
                                        {d.code}{" "}
                                        <span className="text-xs text-gray-400">{d.active ? "active" : "archived"}</span>
                                    </div>
                                    <div className="text-sm text-gray-600">
                                        {d.type === "fixed"
                                            ? `${(d.value / 100).toFixed(2)} ${d.currency}`
                                            : `${d.value}%`}{" "}
                                        • used: {d.used ?? 0}
                                        {d.maxUses != null ? ` / ${d.maxUses}` : ""} • expires:{" "}
                                        {d.expiresAt ? new Date(d.expiresAt).toLocaleDateString() : "never"}
                                    </div>
                                </div>

                                <div className="flex gap-2">
                                    <button onClick={() => setEditing(d as Discount)} className="px-2 py-1 bg-white/10 rounded">Edit</button>
                                    {d.active ? (
                                        <button onClick={() => onArchiveClick(d as Discount)} className="px-2 py-1 bg-yellow-600 rounded text-black">Archive</button>
                                    ) : (
                                        <button onClick={() => restore(d as Discount)} className="px-2 py-1 bg-green-600 rounded text-white">Restore</button>
                                    )}
                                    <button onClick={() => onDeleteClick(d as Discount)} className="px-2 py-1 bg-red-600 rounded text-white">Delete</button>
                                </div>
                            </div>
                        ))}
                        <LoadMoreSentinel
